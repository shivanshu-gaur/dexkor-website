...
$("leadSubmit").addEventListener("click",async function(){
  ...
  const payload={name:name,first_name:fn,email:email,brand:$("iBrand").value.trim(),flow:FLOWKEY,markup_pct:s.m,monthly_messages:s.v,platform:s.p,monthly_leak:s.leak,ts:new Date().toISOString()};
