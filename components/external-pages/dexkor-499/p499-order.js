...
(function(){const q=new URLSearchParams(location.search);const leak=parseInt(q.get("leak"))||0;const plat=(q.get("p")||"").slice(0,22);
 const el=document.getElementById("bkLeak");
 if(el&&leak){el.innerHTML='Your check showed roughly <b>'+inr(leak)+'</b> a month leaking'+(plat?(' on '+plat):'')+'. We confirm the exact figure '+(document.body.dataset.kind==="order"?'before you pay.':'on the call.');el.style.display="block";}
})();


// Razorpay hosted payment button. Injected from JS because a <script> placed inside
// dangerouslySetInnerHTML never executes; creating it in the real DOM makes the browser
// run payment-button.js, and Razorpay renders its button into the form.
(function(){var wrap=document.getElementById("rzpBtnWrap");if(!wrap||wrap.querySelector("form"))return;var form=document.createElement("form");var s=document.createElement("script");s.src="https://checkout.razorpay.com/v1/payment-button.js";s.async=true;s.setAttribute("data-payment_button_id","pl_T8WGVWfI3LospV");form.appendChild(s);wrap.appendChild(form);})();
}
